...
Process_Info getProcessInfo
    = { 0 }; // Temporary struct for collecting process info
 ...
 */
int
getPropertyFromFile (const char *filename, const char *property,
 ...
 */
size_t
getMacAddress (const char *iface, char *macAddress, size_t szBufSize)
{
 ...
 */
int
getPIDByProcessName (const char *procName, unsigned int *pidOut)
{
 ...
 */
int
fillProcessStatFields (unsigned pid, Process_Info *info, unsigned *flagsOut)
{
  ...
  sscanf (after, "%*c %*d %*d %*d %*d %*d %u %u %*u %u %*u %lu %lu", &flags,
 ...
 */
int
parseConfig (const char *configPath, Config_Data *config)
{
  const char *dot = strrchr (configPath, '.');
  if (!dot
      || (strcmp (dot, ".txt") != 0 && strcmp (dot, ".cfg") != 0
          && strcmp (dot, ".conf") != 0))
    {
      ...
      *eq = '\0';
      ...
      /*
              ...
              if (!config->outputFile) {
                      ...
                      snprintf(defaultFile, sizeof(defaultFile),
      "/tmp/%s_%lu_%s", mac, timenow, CSV_FILE_NAME); config->outputFile =
      defaultFile;
              }
      }
      */
 ...
 */
void
writeProcessInfo (unsigned noOfPids, FILE *output)
{
  ...
  while (tmp)
    {
      printf ("%d,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%u,%s\n", i++, tmp->rssTotal,
              tmp->pssTotal, tmp->shared_clean_total, tmp->private_dirty_total,
              tmp->swap_pss_total, tmp->majFaults, tmp->cputime, tmp->pid,
              tmp->name);
      fprintf (output, "%u,%s,%lu,%lu,%lu,%lu,%lu,%lu,%lu\n", tmp->pid,
               tmp->name, tmp->rssTotal, tmp->pssTotal,
               tmp->shared_clean_total, tmp->private_dirty_total,
               tmp->swap_pss_total, tmp->majFaults, tmp->cputime);
      tofree = tmp;
      tmp = tmp->next;
      free (tofree);
    }
 ...
 */
void
addProcessInfo (Process_Info *addPInfo)
{
                  ...
                  if (expect_rss)
                      ...
                      if (sscanf (tmp, "Rss: %u kB", &rss))
                        {
                          if (rss)
                            {
                              PRINT_DBG_SCANNED (
                                  "Read Rss (%u) after %u/%u lines --> %s\r",
                                  rss, skipped, lines_To_skip, tmp);
                  ...
                  else if (expect_pss)
                    {
                      ...
                      if (sscanf (tmp, "Pss: %u kB", &pss))
                        {
                          PRINT_DBG_SCANNED (
                              "Read Pss (%u) after %u/%u lines  --> %s\r", pss,
                              skipped, lines_To_skip, tmp);
                  ...
                  else if (expect_shared_clean)
                    {
                      ...
                      if (sscanf (tmp, "Shared_Clean: %u kB", &shared_clean))
                        {
                          PRINT_DBG_SCANNED (
                              "Read shared_clean (%u)  %u/%u lines --> %s\r",
                              shared_clean, skipped, lines_To_skip, tmp);
                          ...
                          if (!prev_pss)
                            {
                              ...
                              lines_To_skip
                                  = linesToSkipForSwapPss
                                        ? linesToSkipForDirtyPrivate
                                              + linesToSkipForSwapPss
                                              + linesToSkipForRollover - 2
                                        : linesToSkipForDirtyPrivate
                                              + linesToSkipForRollover - 1;
                            }
                          else
                            {
                      ...
                      if (sscanf (tmp, "Private_Dirty: %u kB", &private_dirty))
                        {
                          ...
                          PRINT_DBG_SCANNED (
                              "Read private_dirty (%u)  %u/%u lines --> %s\r",
                              private_dirty, skipped, lines_To_skip, tmp);
                      ...
                      if (sscanf (tmp, "SwapPss: %u kB", &swap_pss))
                        {
                          ...
                          expect_rss = 1;
                          PRINT_DBG_SCANNED (
                              "Read swap_pss (%u)  %u/%u lines --> %s\r",
                              swap_pss, skipped, lines_To_skip, tmp);
              ...
              if (!linesToSkipForRss)
                  ...
                  if (sscanf (tmp, "Rss: %u kB", &rss))
                    {
                      ...
                      PRINT_DBG_INITIAL (
                          "After %u lines Read Rss (%u) in --> %s\r",
                          linesToSkipForRss, rss, tmp);
                      continue;
                    }
                  else
                    {
                  ...
                  if (sscanf (tmp, "Pss: %u kB", &pss))
                    {
                      ...
                      PRINT_DBG_INITIAL (
                          "After %u lines Read Pss (%u) in --> %s\r",
                          linesToSkipForPss, pss, tmp);
                      continue;
                    }
                  else
                    {
              ...
              else if (!linesToSkipForSharedClean)
                {
                  if (sscanf (tmp, "Shared_Clean: %u kB", &shared_clean))
                    {
                      getProcessInfo.shared_clean_total += shared_clean;
                      linesToSkipForSharedClean
                          = linesSkippedForSharedClean + 1;
                      PRINT_DBG_INITIAL (
                          "After %u lines Read Shared_Clean (%u) in --> %s\r",
                  ...
                  if (sscanf (tmp, "Private_Dirty: %u kB", &private_dirty))
                    {
                      getProcessInfo.private_dirty_total += private_dirty;
                      linesToSkipForDirtyPrivate
                          = linesSkippedForDirtyPrivate + 1;
                      PRINT_DBG_INITIAL (
                          "After %u lines Read DirtyPrivate (%u) in --> %s\r",
                  ...
                  if (sscanf (tmp, "SwapPss: %u kB", &swap_pss))
                    {
                      ...
                      PRINT_DBG_INITIAL (
                          "After %u lines Read Swap_Pss (%u) in --> %s\r",
                          linesToSkipForSwapPss, swap_pss, tmp);
                      continue;
                    }
                  else
                    {
                      ...
                      if (sscanf (tmp, "Rss: %u kB", &rss))
                        {
                          ...
                          PRINT_DBG_INITIAL (
                              "After %u lines Read Rss (%u) in --> %s\r",
                              linesToSkipForRollover, rss, tmp);
                          lines_To_skip = linesToSkipForPss;
                          expect_pss = 1;
                          expect_rss = 0;
                          linesToSkipIfRss0 = linesToSkipForPss
                                              + linesToSkipForSharedClean
                                              + linesToSkipForDirtyPrivate
 ...
 */
void
printHelpAndUsage (char *argv[], bool moreInfo)
{
  printf ("Usage: %s [OPTIONS]\n", XMEM_BIN);
  printf ("A lightweight, configurable tool for collecting detailed system "
          "and per-process memory and CPU statistics.\n");

  printf ("\nOptions:\n");
  printf ("  -a, --all			Include kernel threads for process "
          "monitoring\n");
  printf ("  -c, --config <dir>		Path to configuration directory with "
          "%s extension\n",
          CONFIG_EXTN);
  printf ("  -o, --output <directory>	Output directory for generated CSV "
          "files (default: %s)\n",
          DEFAULT_OUT_DIR);
  printf ("      --interval <seconds>	Interval in seconds between "
          "iterations (overrides config)\n");
  printf ("      --iterations <count>	Number of iterations to run "
          "(overrides config)\n");
  printf ("  -h, --help			Show this help message and exit\n");
  printf ("  -t, --test			Run in test mode with a generated "
          "minimal config\n");
  printf ("\n");
  ...
  if (moreInfo)
    {
      printf ("\nPrecedence:\n");
      printf ("  Command-line arguments override config file values, which "
              "override built-in defaults.\n");
      printf ("\nDefault behavior (no flags):\n");
      printf ("  - Runs 1 iteration, interval 0s, monitors all processes, log "
              "level INFO\n");
      ...
      printf ("  - If both interval and iterations are set via CLI, both are "
              "used.\n");
      printf ("  - If only one is set, the other uses its default or config "
              "value.\n");
 ...
 */
int
collectSystemMemoryStats (bool includeKthreads, const char *outDir,
  ...
  for (int iter = 0; iter < iterations; iter++)
    {
      ...
      snprintf (outputfile, sizeof (outputfile), "%s/%s_%s_%s", dir, mac,
                timestamp,
                CSV_FILE_NAME); // add iteration number here if required
      ...
      unsigned long rssTotal = 0, pssTotal = 0, shared_clean_total = 0,
                    private_dirty_total = 0, swap_pss_total = 0;
                  ...
                  if (pid > 0)
                    {
                      ...
                      if (!fillProcessStatFields (pid, &getProcessInfo,
                                                  &flags))
                        {
                          printf ("Failed to fill process stat fields for PID "
                                  "%d\n",
                                  pid);
                          continue;
                        }
                      ...
                      if (!getProcessInfos (pid))
                        {
                          ...
                          shared_clean_total
                              += getProcessInfo.shared_clean_total;
                          private_dirty_total
                              += getProcessInfo.private_dirty_total;
      ...
      printf ("Processed %u processes\nRSS_Total %lu\nPSS_Total "
              ...
              noOfPids, rssTotal, pssTotal, shared_clean_total,
              private_dirty_total);
      ...
      fprintf (output, "0,Total,%lu,%lu,%lu,%lu,%lu,0,0,0\n", rssTotal,
               pssTotal, shared_clean_total, private_dirty_total,
               swap_pss_total);
      fclose (output);
...
int
handleConfigMode (const char *confFile, const char *cli_out_dir,
                  int cli_iterations, int cli_interval, bool enableKThreads)
{
  Config_Data config;
  ...
  const char *final_out_dir
      = (cli_out_dir[0] && strcmp (cli_out_dir, DEFAULT_OUT_DIR) != 0)
            ? cli_out_dir
  ...
  printf ("%s Config loaded:\n whitelist=%p\n count=%u\n outDir=%s\n "
          "iterations=%u\n interval=%u\n logLevel=%s\n",
          confFile, config.whitelist, config.whiteListCount, config.outputDir,
          final_iterations, final_interval, config.logLevel);
  ...
  for (int iter = 0; iter < final_iterations; iter++)
    {
      ...
      snprintf (outputFilePath, sizeof (outputFilePath), "%s/%s_%s_%s",
                final_out_dir, mac, timestamp, CSV_FILE_NAME);
      ...
      fprintf (output, "0,Total,%lu,%lu,%lu,%lu\n", rssTotal, pssTotal,
               shared_clean_total, private_dirty_total);
      fclose (output);
...
main (int argc, char *argv[])
{
  ...
  if (isConfigPresent)
    {
      handleConfigMode (confFile, out_dir, cli_iterations, cli_interval,
                        enableKThreads);
    }
  else if (isSystemWide)
    {
      int final_iterations
          = (cli_iterations > 0) ? cli_iterations : DEFAULT_ITERATIONS;
      int final_interval
          = (cli_interval > 0) ? cli_interval : DEFAULT_INTERVAL;
      collectSystemMemoryStats (enableKThreads, out_dir, final_iterations,
                                final_interval);
    }
  else if (isTestMode)
    {
