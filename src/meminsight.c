...
static int mi_upload_t2_files(const char *outDir)
{
    ...
    if (!certsel) {
        fprintf(stderr, "[MemInsight] Upload: rdkcertselector_new failed, skipping upload.\n");
        curl_global_cleanup();
        return 0;
    }
    printf("[MemInsight] Upload: Using rdkcertselector for mTLS.\n");
#else
    fprintf(stderr, "[MemInsight] Upload: No cert selector available (not built with --enable-certselector), skipping.\n");
    curl_global_cleanup();
    return 0;
#endif
...
/**
 * Reads the aggregate cpu line from PROC_STAT_FILE and writes the 8 CPU fields
...
#define CPUSTAT_FIELD_COUNT 8
    ...
    if (parsed < CPUSTAT_FIELD_COUNT) {
        PRINT_ERROR("Warning: %s cpu line has only %d fields (expected %d), zero-filling rest\n",
                    PROC_STAT_FILE, parsed, CPUSTAT_FIELD_COUNT);
    }
...
/**
 * @brief Add system-wide CPU stats from PROC_STAT_FILE to a JSON root object.
 *
